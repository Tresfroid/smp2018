...
class CNNText_Inception(nn.Module):
    ...
    def forward(self, content):
        ...
        max_pool1d = nn.MaxPool1d(kernel_size=seq_len)
        output = max_pool1d(self.conv(content.permute(0, 2, 1)))
...
def train_model(model,
    ...
    for epoch_i in range(num_epoch):
        ...
        if mode != 'new':
            model.load_state_dict(torch.load('best_model/' + model_name + '_params.pkl'))
        model.train(True)
...
def main():
    ...
    train_model(model, optimizer, loss_function, 50, train_batch, dev_batch, vocab, plot_every=100, lr=1e-2,
                lr_decay=0.5, decay_tol=-1, best_dev_f1=0., optim='sgd', model_name='CNNText_Inception300_sgd', mode='new', cuda=3)  # cuda 0
